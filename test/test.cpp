// g++ -o breakout breakout.cpp `sdl2-config --cflags --libs` -lSDL2_ttf
#include <SDL2/SDL.h>
...
int main(int argc, char** argv) {
	...
	SDL_Event event;
	SDL_Rect rect;
	// paddel(moving bar) structure
	SDL_Point paddelPos = {280, 468};
	...
	TTF_Init();
	TTF_Font* font = TTF_OpenFont("/usr/share/fonts/truetype/ubuntu/Ubuntu-MI.ttf", 24);
	...
	while (isRunning) {
		...
		for (float w=0; w<rad*2; w++) {
			for (float h=0; h<rad*2; h++) {
				float dx = rad - w;
				float dy = rad - h;
				if ((dx * dx + dy * dy) <= (rad * rad)) SDL_RenderDrawPoint(renderer, cirCenter.x + dx, cirCenter.y + dy);
			}
		}
		...
		if (checkCollision(rect, cirCenter, rad)) vY = -1;
		...
		while (SDL_PollEvent(&event)) {
			...
			switch (event.type) {
				...
				case SDL_MOUSEMOTION:
					paddelPos.x = event.motion.x - blockWidth / 2;
					break;
			}

			if (paddelPos.x <= 0) paddelPos.x = 0;
			else if (paddelPos.x + blockWidth >= SCREEN_WIDTH) paddelPos.x = SCREEN_WIDTH - blockWidth;
